...
**Seven slots** run at half-hour steps from 23:17 to 02:17 UTC (07:17-10:17 Taipei). GitHub's scheduler is best-effort and has missed badly: 2026-08-26 was dropped during an Actions incident, 08-27 fired five hours late, and on 08-28 two slots were skipped with no incident, no failure and no notification while the same repository's CI ran normally. Two slots proved insufficient because they queue through the same subsystem and are dropped together, so this trades cheap wasted runs for more chances to land. It is a probability play, not a guarantee: if GitHub declines to schedule the workflow for a whole window, every slot in it goes too. The only way to remove that dependency is an external trigger calling the `workflow_dispatch` API.

Only the first success actually sends. Before running, a scheduled run looks for a successful run **that pushed** on the same Taipei date and stands down if it finds one; the rest cost about a minute of free runner time each. Delivery is identified by the run title (`run-name` records the mode), so a manual catch-up counts and a `dry-run` preview does not — an earlier version only considered scheduled runs, and a manual catch-up followed by a late slot sent the digest twice. A run that started but **failed** is not delivery, so a later slot retries it. The check fails open: if it cannot answer, the digest sends anyway, because a duplicate is far cheaper than a suppressed message. Manual dispatches never skip.
